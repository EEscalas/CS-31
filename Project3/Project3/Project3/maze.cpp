...
bool isRouteWellFormed(string route)
{
	for (int position = 0; position != route.size(); position++)
	{
		if (isdigit(route[0]))
			return false;
		if (!isalpha(route[position]) && !isdigit(route[position]))
			return false;
...
int navigateSegment(int r, int c, char dir, int maxSteps)
{
	...
	if (tolower(dir) != 'n' && tolower(dir) != 's' && tolower(dir) != 'e' && tolower(dir) != 'w')
		return -1;
	if (maxSteps < 0)
		return -1;
	if (r > getRows() || c > getCols() || r <= 0 || c <= 0)		// if the point given is outside of the grid, point not valid
		return -1;
...
int navigateRoute(int sr, int sc, int er, int ec, string route, int& nsteps)
{
	if (! isRouteWellFormed(route) || !clearPosition(sr, sc) || !clearPosition(er, ec))
	{
		return 2;
	}
	...
	string newRoute;			// modified route where all lower case letters
	while (k != route.size())	// go through every character of the route string
	{
		...
		if (isalpha(route[k] && route[k + 1] == '0'))
		{
			continue;
		}
		if (isalpha(route[k]))
	...
	if (!isRouteWellFormed(newRoute))
	{
		return 2;
	}

	int m = 0;				// set me to zero, m is the position in the newRoute
	...
	int stepsMoved = 0;		// stepsMoved will count up the possible number of steps
	int possible_steps = 0;	// an integer set to the value of navigateSegment (makes later computations to move point (mr, mc) simpler)

	while (m != newRoute.size())	// go through every character in the newRoute
	{
		possible_steps = navigateSegment(mr, mc, newRoute[m], 1);
		if (possible_steps < 1)				// if navigateSegment returns a number less than one
		{
			nsteps = stepsMoved;			// return the number of steps you have taken so far
			return 3;						// return 3 because you hit a wall or fell off the grid
		}
		else
	...
	if (mr == er && mc == ec)				// if the final point is the end point
	{
		nsteps = stepsMoved;
		return 0;
	}
